...
const CollapsibleItemSection = ({ index, itemData, handleChange, expanded, updateItem, removeItem, pmplData }) => {
    ...
    useEffect(() => {
        const fetchStockAndGodown = async () => {
            const stockRes = await fetch("http://localhost:80/api/stock");
            const stockData = await stockRes.json();

            const godownRes = await fetch("http://localhost:80/api/dbf/godown.json");
    ...
    const handleItemChange = (event, newValue) => {
        ...
        if (selectedItem) {
            ...
            const totalStock = Object.values(stockList[selectedItem.CODE] || {}).reduce((acc, val) => acc + val, 0);
    ...
    const calculateAmounts = (data) => {
        ...
        const netAmount = amount + (amount * (data.cess / 100)) - (amount * (data.cd / 100)) - (amount * (data.sch / 100));
        return {
            ...data,
            ...
            <AccordionDetails>
                <Grid container spacing={2}>
                    ...
                    <Grid item xs={12} sm={3}>
                        <Autocomplete
                            options={pmplData.filter(item => item.STK > 0).map(item => ({ label: `${item.CODE} | ${item.PRODUCT}`, value: item.CODE }))}
                    ...
                    <Grid item xs={12} sm={3}>
                        <Autocomplete
                            ...
                            renderInput={(params) => (
                                <TextField {...params} label="Godown" fullWidth />
                            )}
                        />
                    </Grid>
                    <Grid item xs={12} sm={3}>
