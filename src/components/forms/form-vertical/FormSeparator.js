...
function FormSeparator() {
    ...
    const [items, setItems] = useState([
        { item: '', godown: '', unit: '', stock: '', pack: '', gst: '', pcBx: '', mrp: '', rate: '', qty: '', cess: '', schRs: '', sch: '', cd: '', amount: '', netAmount: '' }
    ]);
    ...
    useEffect(() => {
        const fetchOptions = async () => {
            const res = await fetch("http://localhost:80/cmpl");
            const data = await res.json();

            const pmplRes = await fetch("http://localhost:80/api/dbf/pmpl.json");
            const pmplData = await pmplRes.json();
            setPmplData(pmplData);
            const balanceRes = await fetch("http://localhost:80/json/balance");
            const balanceData = await balanceRes.json();

            const getBalance = (C_CODE) => balanceData.data.find(user => user.partycode === C_CODE)?.result || 0;
    ...
    const addItem = () => {
        setItems([...items, { item: '', godown: '', unit: '', stock: '', pack: '', gst: '', pcBx: '', mrp: '', rate: '', qty: '', cess: '', schRs: '', sch: '', cd: '', amount: '', netAmount: '' }]);
